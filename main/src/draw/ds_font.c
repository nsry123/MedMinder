...
static uint8_t *user_font_getdata(uint32_t offset, uint32_t size) {
    static uint8_t first_in = 1;
    if (first_in == 1) {
        partition_res = esp_partition_find_first(FLASH_ADDR_DATA, FLASH_ADDR_NVS, NULL);
        first_in = 0;
        if (partition_res == NULL) {
            printf("Failed to open partition for reading\n");
            return NULL;
        } else {
            printf("Successfully opened partition for reading\n");
        }
    }

    // Check if the read is within the partition size
    if (offset + size > partition_res->size) {
        printf("Error: Read exceeds partition size. Offset: %u, Size: %u, Partition Size: %u\n", offset, size, partition_res->size);
        return NULL;
    }

    esp_err_t res = esp_partition_read(partition_res, offset, __g_font_buf, size);
    if (res != ESP_OK) {
        printf("Failed to read at offset %x\n", offset);
        return NULL;
    }
    return __g_font_buf;
}
...
static const uint8_t *user_font_get_bitmap(uint32_t pos, uint32_t size) {
    const uint8_t *data = user_font_getdata(pos + sizeof(glyph_dsc_t), size);
    if (data == NULL) {
        printf("Error: Failed to get bitmap data at position %u\n", pos);
    }
    return data;
}
...
//         const uint8_t *font_data = user_font_get_bitmap(pos, size);
//         if (font_data == NULL) {
//             printf("Error: Failed to get font data for letter U+%04X.\n", letter);
//             return 0;
//         }

//         memcpy(bitmap_buf, font_data, size);
//         return size;
//     } else {
//         printf("Error: Glyph not found for letter U+%04X.\n", letter);
//     }
//     return 0;
// }
