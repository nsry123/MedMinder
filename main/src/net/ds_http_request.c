...
static void cjson_time_info(char *text)
{
    ...
    update_system_time(hour,minute,second);//更新系统时间
}
...
void cjson_weather_info(char *text)
{
    ...
    //截取有效json
    char *index=strchr(text,'{');
    strcpy(text,index);//把获取到的json复制到text中
    root = cJSON_Parse(text);
    if(root!=NULL)
    {
...
void http_weather_get(){
	...
	if(err == ESP_OK)
	{
		ESP_LOGI(TAG, "Status = %d, content_length = %d",
				...
				(int) esp_http_client_get_content_length(weather_client));//数据长度
	}
...
static void http_request_task(void *pvParameters)//http请求事件
{
    while(1) {//每2秒刷新一次
        HTTP_REQUEST_TYPE_E evt;
